"""Ollama-backed OCR provider for the OCR + Vision subsystem (Phase 5C).

This provider implements the :class:`core.vision.ocr.OCRProvider` interface using
a local Ollama server with a vision-capable model (e.g. ``qwen2.5vl:3b``) that
can transcribe text from images. The server MUST run on the same machine
(loopback) — the provider refuses non-loopback base URLs at construction time,
preserving the air-gapped guarantee of the platform.
...
into the request and are never logged. The extracted text is never logged.
"""
...
from core.vision.ocr import SUPPORTED_OCR_CONTENT_TYPES, OCRProvider
...
_logger = logging.getLogger("sovereign-ai.vision.ollama_ocr")
...
#: Prompt used to extract text from an image. The model is asked to return only
#: the text content, preserving line breaks, with no surrounding commentary.
_OCR_PROMPT: str = (
    "Extract all text from this image. Return only the text content, preserving "
    "line breaks and spacing as they appear in the image. Do not add any "
...
class OllamaOCRProvider(OCRProvider):
    """OCR provider backed by a local Ollama server.

    Uses a vision-capable model to transcribe text from a single image or a
    multi-page document. The full transcribed text is returned in a single block
    (structured line/word metadata is not produced by this provider).

    The provider is **synchronous** — ``recognize`` blocks until the local
    Ollama server responds. This matches the :class:`OCRProvider` interface
    (the agent tool bridge calls these methods without ``await``).
    """
    ...
    def default_model(self) -> Optional[str]:
        """The configured default model, or ``None``."""
    ...
    def recognize(self, image: ImageInput) -> OCRResult:
        ...
        Returns:
            An :class:`OCRResult` with the transcribed text in a single block.
        ...
        try:
            ...
            _logger.info(
                "ollama_ocr.recognize  model=%s  latency_ms=%.1f",
                model,
                latency_ms,
            )
        ...
        # Ollama does not report per-line/per-word confidence or boxes. We
        # surface the transcribed text as a single block so downstream callers
        # (RAG bridge, agent tool) still receive consistent structure.
        block = OCRBlock(
            text=full_text,
            confidence=None,
            lines=(
                OCRLine(
                    ...
                    confidence=None,
                    words=(
                        OCRWord(text=full_text, confidence=None),
                    ),
                ),
            ),
            block_type="text",
    ...
    def recognize_document(
        ...
        Calls :meth:`recognize` for each page in order.
